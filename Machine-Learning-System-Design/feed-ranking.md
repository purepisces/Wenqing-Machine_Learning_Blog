...
> 
> **Position Match:** Means that the item which the model ranked highest (or in another specific rank) based on its predicted click probability is the same item that the user actually clicked on.
...
<img src="Feed_Ranking_High_level_design.png" alt="Feed_Ranking_High_level_design" width="400" height="300"/>
...
1. Client sends feed request to Application Server
<img src="client_send_request.png" alt="client_send_request" width="400" height="300"/>
2. Application Server sends feed request to Feed Retrieval Service
<img src="application_server_send_request.png" alt="application_server_send_request" width="400" height="300"/>
3. Feed Retrieval Service selects most relevant feeds from Item Store
<img src="feed_retrieval_select.png" alt="feed_retrieval_select" width="400" height="300"/>
4. Feed Retrieval Service sends feed ranks request to Feed Ranking Service
<img src="feed_retrieval_send_request.png" alt="feed_retrieval_send_request" width="400" height="300"/>
5. Feed Ranking Service gets the latest ML model, gets the right features from Feature Store
<img src="feed_ranking_get.png" alt="feed_ranking_get" width="400" height="300"/>
6. Feed Ranking Service scores each feeds and returns to Feed Retrieval Service and Application Server. Application server sorts feeds by rankings and return to client
<img src="return_feeds.png" alt="return_feeds" width="400" height="300"/>


#### Flow of the System

1. A user visits the LinkedIn homepage and requests an Application Server for feeds. The Application Server sends feed requests to the Feed Service.
2. Feed Service gets the latest model from Model Repos, gets the correct features from the Feature Store, and all the feeds from the ItemStore. Feed Service will provide features for the Model to get predictions.
3. The Model returns recommended feeds sorted by click-through rate likelihood.
